    ...
    <div className="relative bg-white rounded-lg shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden z-50">
      ...
      <div className="bg-gray-50 px-6 py-3 border-b flex justify-between items-center">
        <div className="text-sm text-gray-600">
          <span className="font-semibold">{attendees.filter(e => e.event === event?.title).length}</span> attendees registered
          {event?.capacity && (
            <span className="ml-2">
      ...
      <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
        ...
        ) : attendees.filter(e => e.event === event?.title).length === 0 ? (
          <div className="text-center py-12">
          ...
          <div className="overflow-x-auto">
            <table className="w-full table-auto">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {attendees.filter(e => e.event === event.title).map((attendee, index) => (
                  <tr key={attendee.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getMealPreferenceColor(attendee.mealPreference)}`}>
